...
# Per-target success lines are past-tense ("written ->", "copied ->") and some
# carry counts computed during the build, so under --dry-run they would claim
# work that did not happen. Suppress them: do_write / do_copy_dir already print
# a "DRY-RUN would write/copy: <target>" line carrying the same path.
ok()    {
  [[ -n "${DRY_RUN:-}" ]] && return 0
...
# Provenance line emitted into single-file artefacts that land on a filename
# other tools also claim (AGENTS.md, CONVENTIONS.md). Lets a re-run recognise
# its own output and overwrite in place instead of forking to a suffixed name.
PROVENANCE="<!-- cloud-finops-skill: generated by install.sh - https://github.com/OptimNow/cloud-finops-skills -->"
ALL_TOOLS=(claude-code claude-projects cursor windsurf chatgpt gemini gemini-cli codex aider copilot kiro mcp)

# The named waste patterns the playbook library covers, as one comma-separated
# list. Emitted verbatim into the "Named waste pattern" routing row of BOTH
# ChatGPT instruction variants - the per-reference one (routing to
# playbook-<slug>.md) and the grouped one (routing to a section of
# playbooks.md). It was ~330 words duplicated byte-for-byte between the two
# builders, which meant every new playbook had to be added twice and an
# omission in one variant was invisible: the grouped build simply stopped
# routing to a pattern the default build still knew about.
#
# The routing rows live inside quoted heredocs (so the backticks and <slug>
# placeholders around them stay literal), so each builder breaks its heredoc
# for one printf rather than switching to an unquoted heredoc.
NAMED_WASTE_PATTERNS="zombie NAT, snapshot sprawl, idle ELB, cross-AZ egress, oversized RDS, orphan EBS, orphan Azure disks, App Service overprovisioning, Log Analytics ingestion sprawl, idle Azure SQL, idle GKE Autopilot, orphan Persistent Disks, Cloud Functions cold starts, schedule blindness, untagged spend drift, idle SageMaker endpoint, always-on SageMaker notebook, SageMaker endpoint sprawl / MME consolidation, oversized GPU instance, multi-GPU underutilized, MIG candidate, GPU for CPU-bound workload, outdated GPU generation, agent-loop flat-line burn, coding-agent token waste, expiring commitment without a renewal decision, unused Azure reservation, GCP CUD mismatch, S3 incomplete multipart uploads, S3 noncurrent version sprawl, S3 cold data in Standard, NAT-to-gateway-endpoint substitution, idle Azure VM (stopped not deallocated), orphaned Azure public IPs and NICs, Azure snapshot sprawl, gp2-to-gp3 migration, Graviton candidate"

# Emit the "Named waste pattern" routing row. $1 is the knowledge-file column,
# which is the only thing that differs between the two variants.
emit_named_pattern_row() {
  printf '| Named waste pattern (%s) | %s |\n' "$NAMED_WASTE_PATTERNS" "$1"
}

# ---- state ----
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" 2>/dev/null && pwd || pwd)"
SRC_DIR=""
# Where resolve_source clones the repo when there is no local checkout, cleaned
# up on exit. Named CLONE_TMP, not TMPDIR: TMPDIR is a standard exported
# variable that every child process (git, mktemp, rsync, python) reads to find
# the system temp directory, and this script used to overwrite it with "" and
# then with its own clone path. Renaming leaves the caller's TMPDIR alone - and
# lets the `mktemp -d` below honour it, which it could not while we were the
# ones setting it.
CLONE_TMP=""
TOOL=""
DRY_RUN=""
...
cleanup() { [[ -n "$CLONE_TMP" ]] && rm -rf "$CLONE_TMP" 2>/dev/null || true; }
trap cleanup EXIT
...
# $1 - exit code (default 0). The unknown-option branch must pass 1: printing
# the help text is the right response to a typo, but exiting 0 while doing it
# tells every caller the run succeeded. `./install.sh --bogus-flag` used to
# print "Unknown option" and exit 0, so a CI step or a wrapper script that
# checked the status saw a clean install that had installed nothing.
usage() {
...
resolve_source() {
  if [[ -f "$SCRIPT_DIR/skills/cloud-finops/SKILL.md" ]]; then
    ...
    CLONE_TMP=$(mktemp -d 2>/dev/null || mktemp -d -t 'finops-skill')
    git clone --depth 1 --quiet "$REPO_URL" "$CLONE_TMP/cloud-finops-skills"
    SRC_DIR="$CLONE_TMP/cloud-finops-skills"
  fi
}

# Strip the YAML frontmatter (between the first two --- lines) from a markdown file.
#
# `sub(/\r$/,"")` on every line before matching, the same handling
# scripts/fcp-coverage.sh and scripts/playbook-coverage.sh already use. On a
# CRLF checkout - WSL running against a Windows clone with core.autocrlf=true,
# a combination this script explicitly supports - the fence line is `---\r`,
# which `/^---$/` never matches. The frontmatter delimiter is then never seen,
# f stays 0, nothing is printed, and every generated artefact silently loses
# the SKILL.md body. (The pre-fix behaviour was worse in the emit paths below,
# where the whole YAML block leaked into the artefact instead.)
strip_frontmatter() {
  awk 'BEGIN{f=0} {sub(/\r$/,"")} /^---$/{f++; if(f<=2) next} f>=2{print}' "$1"
}
...
emit_reference_index() {
  ...
  for ref in "$SRC_DIR/skills/cloud-finops/references"/*.md; do
    ...
    desc=$(awk '{sub(/\r$/,"")} /^>/{sub(/^> ?/,""); print; exit}' "$ref" | head -c 220)
    echo "- \`$name\` - $desc"
  done
}
...
EOF
    ...
    for pb in "$SRC_DIR/skills/cloud-finops/playbooks"/*.md; do
      ...
      # ride along in every facet value and land mid-line in the generated index.
      scope=$(awk -F': *' '{sub(/\r$/,"")} /^scope:/{print $2; exit}' "$pb")
      cat=$(awk -F': *' '{sub(/\r$/,"")} /^waste_category:/{print $2; exit}' "$pb")
      conf=$(awk -F': *' '{sub(/\r$/,"")} /^confidence:/{print $2; exit}' "$pb")
...
# True when an existing file carries our provenance line, i.e. a previous run
# of this installer wrote it and overwriting in place is safe. Only the head of
# the file is checked - the marker is emitted in the header block.
is_ours() {
  head -n 20 "$1" 2>/dev/null | grep -qF "cloud-finops-skill: generated by install.sh"
}
...
do_copy_dir() {
  ...
  # Stage into a sibling temp dir and swap, so an interrupt mid-copy cannot
  # leave the user with their previous install deleted and nothing in its place.
  local staging="$dest.tmp.$$"
  rm -rf "$staging"
  if command -v rsync >/dev/null 2>&1; then
    mkdir -p "$staging"
    rsync -a --exclude='.claude' --exclude='.backups' --exclude='.git' "$src/" "$staging/"
  else
    cp -r "$src" "$staging"
    rm -rf "$staging/.claude" "$staging/.git" 2>/dev/null || true
    find "$staging" -type d -name '.backups' -exec rm -rf {} + 2>/dev/null || true
  fi
  rm -rf "$dest"
  mv "$staging" "$dest"
}
...
# Auto-detect writes files into the user's project, so the signal has to be
# specific to the tool. CONVENTIONS.md is a convention several tools use, and
# `code` on PATH says the user has VS Code - not that they want a Copilot
# instructions file written into this repo. Both were dropped for that reason
# (as was the earlier .github/ heuristic, which fired on every repo with
# workflows or issue templates). Either target is still reachable explicitly
# via --tool aider / --tool copilot.
detect_aider()           { command -v aider >/dev/null 2>&1; }
detect_copilot()         { [[ -f ".github/copilot-instructions.md" ]]; }
...
install_chatgpt() {
  ...
  if [[ -n "$GROUPED" ]]; then
    # Grouped build emits a routing contract that points at the grouped
  ...
  if [[ -n "$GROUPED" ]]; then
    # Grouped build: same domain bundling as Gemini. Produces a handful of
    # thematic knowledge files rather than one per reference - well under any
    # historical ChatGPT Custom GPT cap and easier to upload reliably. Routing
    # contract above already points at the grouped filenames.
  ...
  if [[ -z "$GROUPED" ]]; then
    ...
    dim "    4. Grouped mode: thematic files (aws, azure, gcp, ai, data-platforms,"
...
EOF
  emit_named_pattern_row 'playbook-<slug>.md (e.g. playbook-aws-zombie-nat-gateway.md)'
  cat <<'EOF'
...
EOF
  emit_named_pattern_row 'playbooks.md'
  cat <<'EOF'
...
install_codex() {
  ...
  if [[ -f "$target" ]] && ! is_ours "$target"; then
    warn "Codex: $target exists and was not written by this installer."
    warn "       Writing to AGENTS-cloud-finops.md instead."
...
install_aider() {
  ...
  local target="$default_target"
  if [[ -f "$target" ]] && ! is_ours "$target"; then
    warn "Aider: $target exists and was not written by this installer."
    warn "       Writing to CONVENTIONS-cloud-finops.md instead."
...
install_copilot() {
  local default_target="${DEST_OVERRIDE:-$PWD}/.github/copilot-instructions.md"
  local target="$default_target"
  # The same provenance guard install_codex and install_aider use, and the one
  # target that needed it most. detect_copilot fires only when
  # .github/copilot-instructions.md ALREADY exists, and this function used to
  # call do_write unconditionally - so the single situation auto-detect mode
  # could act on was one where it destroyed a file the user had written by
  # hand. Now a file that is not ours is left alone and the build goes to a
  # suffixed name, exactly as with AGENTS.md and CONVENTIONS.md.
  if [[ -f "$target" ]] && ! is_ours "$target"; then
    warn "Copilot: $target exists and was not written by this installer."
    warn "         Writing to copilot-instructions-cloud-finops.md instead."
    target="${DEST_OVERRIDE:-$PWD}/.github/copilot-instructions-cloud-finops.md"
  fi
  do_write "$target" build_copilot_instructions
  ok "Copilot: instructions written -> $target"
  if [[ "$target" != "$default_target" ]]; then
    dim "  Copilot reads .github/copilot-instructions.md. Merge the generated file"
    dim "  into yours (or rename it when ready) - Copilot will not read it as is."
  fi
...
build_copilot_instructions() {
  # The provenance marker must be inside the first 20 lines - that is the window
  # is_ours() inspects - so it goes directly under the H1, as in the codex and
  # aider builders. Without it a re-run cannot recognise its own output and
  # would fork to a suffixed file on every invocation.
  cat <<EOF
...
install_mcp() {
  ...
  printf "\n"
  printf "${C_BOLD}Step 1${C_RESET} - install the server (Python 3.10+):\n"
  printf "    pip install cloud-finops-mcp\n"
  printf "    ${C_DIM}# or, no install: ${C_RESET}uvx cloud-finops-mcp\n"
  printf "\n"
  printf "${C_BOLD}Step 2${C_RESET} - add to your MCP client config:\n"
  printf "\n"
  printf "  ${C_BOLD}Claude Code${C_RESET}  (user scope - one command, no file editing):\n"
  printf "    claude mcp add --scope user cloud-finops -- cloud-finops-mcp\n"
  printf "\n  ${C_BOLD}Claude Code${C_RESET}  (project scope - .mcp.json at project root):\n"
  cat <<'EOF'
    {
      "mcpServers": {
...
EOF
  dim "    Note: ~/.claude/mcp.json is NOT read by Claude Code. User-scope config"
  dim "    lives in ~/.claude.json - use the command above rather than editing it."
...
EOF
  printf "\n"
  printf "${C_BOLD}Step 3${C_RESET} - restart your client. Verify:\n"
  dim "    Claude Code: /mcp"
...
main() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --help|-h)  usage 0 ;;
      *)          err "Unknown option: $1"; echo; usage 1 ;;
    esac
  done
  ...
  printf "\n"
  if [[ -n "$DRY_RUN" ]]; then
    printf "${C_BLUE}[DRY]${C_RESET} Dry run complete - nothing was written. Targets: ${selected[*]}\n"
  else
    printf "${C_GREEN}[OK]${C_RESET}  Done. Installed for: ${selected[*]}\n"
  fi
