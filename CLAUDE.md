...
│                             cannot drift apart) - all run by the `CI`
│                             workflow - and check-marketplace-version
...
5. **Register the file in the three places that enumerate references.**
   ...
   target can detect one - check `.github/workflows/ci.yml` for such a step
   before relying on it, and treat the ChatGPT table as hand-checked regardless.
